...
serve(async (req) => {
  ...
  try {
    ...
    if (profile.token_balance <= 0) {
      console.log('User has insufficient credits:', profile.token_balance);
      return new Response(
        JSON.stringify({ 
          error: 'Saldo de créditos insuficiente. Por favor, recarregue.',
    ...
    const prompt = `Gere uma foto de produto de alta qualidade: ${prompt_product}. Modelo: ${prompt_model}. Cenário: ${prompt_scene}.${observationsText} Estilo realista, iluminação suave.`;
    ...
    if (updateError) {
      ...
      // Prosseguimos mas logamos o erro; usuário já recebeu a imagem
    }
